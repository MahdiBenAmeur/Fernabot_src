...
from senderActions import mark_message_seen, show_typing, similarity_percentage
...
tree = {
    # Main Menu
    "1": {
        "message": [ "أهلا وسهلا بيك في فرنا بوت 🤖! هاني هنا لنساعدك تلقى فرص التدريب 📚، الخدمة، والتمويل لمشاريعك في فرنانة." , " قول لي شنوا تحب تعرف أكثر وأنا في الخدمة لنعطيك آخر المستجدات! 😊"],
...
def send_message(recipient_id: str, message: dict) -> dict:
    ...
    if "text" in message and type (message["text"]) == list and len (  message["text"]) > 1 :
        for mes in range(len (message["text"])-1):
            payload = {
                "recipient": {"id": recipient_id},
                "message": {"text" : message["text"][mes]}
            }
    ...
    if "quick_replies" in message :
        for payload in message["quick_replies"]:
            last_payloads.append(payload["payload"])
    if recipient_id not in user_dic :
        user_dic[recipient_id] = []
    user_dic[recipient_id] = last_payloads
...
def build_more_option_response(payload: str) -> dict:
    ...
    for index, title in enumerate(action_result["titles"]):
        ...
        if "urls" in action_result and action_result["urls"][index] != "":
        ...
        for entry in data.get('entry', []):
            for event in entry.get('messaging', []):
                ...
                # If user sent a text message
                    ...
                    next_payload = find_next_payload(sender_id , event["message"]["text"])
                    if next_payload in tree:
                        response_message = build_response(next_payload)
                    else :
                        response_message = build_more_option_response(next_payload)
...
def find_next_payload ( id , text ) :
    if id not in user_dic or user_dic[id] == [] :
        return  "1"
    max_score= 0
    max_key = "1"
    for key in user_dic[id] :
        if key not in tree :
            score = similarity_percentage(text , "زيدني" )
        else :
            score = similarity_percentage(text , tree[key]["title"] )
        if score>max_score :
            max_score = score
            max_key = key
    if max_score< 50 :
